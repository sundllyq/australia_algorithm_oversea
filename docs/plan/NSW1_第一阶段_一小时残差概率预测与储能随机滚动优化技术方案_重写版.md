...
### 2.2 从基础闭环到最终评价，按什么顺序研究

研究按“先确认算得对，再检验方法是否更好，最后检查站点约束下是否仍有效”的顺序推进。下图从数据准备画起，箭头统一表示本方案建议的实施先后；各阶段的具体交付与检查条件见第 11.2 节。

```mermaid
flowchart LR
    A["审计历史数据<br/>还原决策时可见的信息"]
    B["建立概率基线<br/>保留原始 P5MIN，用历史误差补上概率"]
    C["跑通并验证基础闭环<br/>只模拟电池与电网交易，核对电量和费用"]
    D["在简单场景中逐项检验方法<br/>比较候选预测模型，按问题检验路径与优化增强"]
    E["加入合成负荷、光伏与站点约束<br/>比较基线和保留的方法，检查费用与资产敏感性"]
    F["冻结配置并做最终评价<br/>在最新合格完整 12 个月上测试"]
    A --> B --> C --> D --> E --> F
```

基础闭环通过检查后，才有可信的参照来判断候选方法是否改善运营价值。这里的检查包括：输入在决策时可见，动作满足物理约束；能根据实际执行功率、效率和上一时点电量重算每一步电池能量；能根据进出口电量、实际 NSW1 RRP、费用与退化参数重算每个五分钟区间的现金流，并核对计算正确。这些都是离线研究假设下的回放记录，不是真实用户的财务结算。

接下来先保持只有电池与电网的简单场景，逐项改变预测或决策方法，便于识别改进来自哪里；再把保留的方法与基线一起放进含负荷和光伏的合成站点，检查本地用电、光伏和进出口限制是否改变结论。合成站点可以用概率基线搭建，不依赖候选模型胜出；图中将它放在方法比较之后，是为了明确本方案的实施主线。

候选模型没有改善时，仍可以用概率基线研究路径和控制，但必须说明研究问题已转为“决策方法能否改善基线”，不能继续声称预测模型有效。下表中的增强方法以及更长预测都不是首个闭环的前置条件。

| 接下来的研究 | 只改变什么 | 保留它需要什么证据 |
| --- | --- | --- |
| 学习预测误差的候选模型 | 用当前特征估计误差，替代粗分组统计 | 相对概率基线改善，且对动作和费用后价值的影响可解释 |
| 历史依赖重排 | 保持每步价格候选，改变哪些值连接在一起 | 路径持续性或风险决策有稳定增量 |
| 场景树优化 | 允许未来观察到新信息后再改变后续动作 | 检查动作没有提前使用未来信息，增量足以覆盖求解成本 |
...
图 3-1 把本轮的信息截止、预测区间和实际执行范围放在同一条时间线上。

![图 3-1：单轮决策的信息截点与十二步预测时序](assets/decision-information-timeline.svg)

读图：09:59 之后才可见的数据不能进入本轮信息快照，即使它在 10:00 前已可见。十二个预测目标结束于 10:05 至 11:00，但当前只执行 `(10:00,10:05]` 的动作；10:05 根据更新后的状态和合格输入重新规划。图中的一分钟延迟是教学假设，十二步锚点是否完整仍需按目标逐一校验。
...
对每个目标重复选择，并保留全部版本。假设本轮 12 个目标均有合格锚点，当前电池能量为 12.5 kWh。荷电状态（State of Charge，SOC）用来表示存电比例。项目的 20 kWh 是 10%–90% SOC 之间的可用能量，对应总能量基数 `20 / (90%−10%) = 25 kWh`，称为推导毛能量。因此当前 SOC 为 `12.5 / 25 = 50%`；完整资产约束见第 7.1 节。
...
优化器必须评估一整小时的动作后果。为了说明路径是什么，假设从本轮分布取出以下三组等权候选，即每个候选在示例中占相同的概率权重；它们是单独的采样示意，不是上一表的 P10/P50/P90。
...
现在下游拿到的是“多条十二步价格路径及其权重”。每条路径代表一种可能发生的价格情景，后文也称为一个场景；权重表示它在计算中的概率份额。这些路径还不是动作计划；在路径上如何充放电，由优化器决定。
...
数据以来源、月份和版本分层保存。原始、标准化、特征、预测、动作和结算产物只追加、不覆盖，并记录每份产物由哪些输入和处理步骤生成，这种来源关系称为数据血缘。表结构每行记录一个预测起点、一个目标区间及其步长，即 issue–target–horizon 长表；增加预测步只需增加行。核心领域结构通过配置支持一般步数。12 步是本次配置，不是唯一允许的数组长度。
...
`α` 是 0 到 1 之间的概率位置，例如 0.9 对应 P90；`Q_E(α)` 和 `Q_Y(α)` 分别是残差与价格在该位置的分位数。第 3 章已演示 `120 + (−40) = 80 A$/MWh`。加法成立是因为当前锚点已知，只平移一个分布。历史训练残差同样必须使用历史当时选中的锚点。
...
首轮从同一个校准 CDF 导出事件概率。M2 先独立评分与检查冲突，暂不强行修改主 CDF；只有让事件概率与主分布保持一致的融合方法在验证期表现稳定，才允许融合。令 `F` 表示当前价格的 CDF，`θ` 为带单位的高价阈值，则 `P(Y<0)=F(0−)`，`P(Y>θ)=1−F(θ)`。这里 `F(0−)` 表示严格低于零的概率，`F(0)` 则包含价格恰好为零的概率；经验分布可能在零处跳升，所以不能混用。
...
路径方法只处理跨时间的连接关系。它不能靠“画得更平滑”证明正确，真实价格也可能跳变；要在模型拟合时段之后的数据上，用路径评分、事件持续性和下游动作检验。后文将这种使用较晚数据的评价称为时间外评价，具体切分见第 9 章。
...
图 6-1 将上面的两步教学例子展开为“读取 PIT、跨模板排名、分配当前候选、连接路径”四个步骤。

![图 6-1：历史 PIT 排名如何重排当前价格候选](assets/pit-rank-reordering.svg)

读图：沿模板 A 看，第一列排名最低，领取 40；第二列排名最高，领取 150，于是形成 `40→150`。其他模板按同一规则领取剩余候选。每列的价格集合和等权概率份额均保持不变，变化的是同一条路径在各步拿到哪些价格。图中只展示两步，连线不是已验证的市场规律，也不表示更平滑的路径更好。
...
J3 按顺序回答三个问题：**哪些历史模板能用？其中哪些起点状态更像现在？选中后怎样用它们生成路径？** 资格检查先确定候选范围，相似度再影响合格候选的入选概率，最后才读取入选模板的 PIT 做重排。

下面用三个历史模板说明这个顺序。假设当前是 10:00 AEST，要为 10:05 至 11:00 结束的十二个五分钟区间生成价格路径。表中仅为教学假设；A、B、C 是本例的模板编号，与 6.3 的排名例子无关。

| 历史模板 | 资格检查：能不能用 | 起点状态与当前的关系 | 在选择中怎样处理 |
| --- | --- | --- | --- |
| A | 后续一小时结果缺失，不合格 | 即使当时的 P5MIN 曲线与近期价格很像现在 | 在资格检查时排除，不进入相似度计算 |
| B | 完整结果在允许的训练截点前已可见，版本、质量与历史 CDF 均合格 | 当时可见的 P5MIN 曲线、近期价格等状态与现在相似 | 进入合格库；相似度使其更容易入选，但不保证必选 |
| C | 与 B 一样合格 | 当时可见状态与现在不太相似，但后来实际发生了尖峰 | 可以作为合格候选；不能因为后来发生尖峰而提高其相似度 |

相似度只比较当前和历史起点各自当时可见的 P5MIN、近期价格、日历、市场状态和质量标记。历史后续结果的可用时间与质量可用于资格检查，事件归属可用于限制同一事件的贡献；后续实际价格、残差和 PIT 数值不能进入相似度距离。

下图从左到右展示同一个例子的处理顺序。箭头表示上一阶段产物交给下一阶段；第一步检查资格并建立合格历史库，当前状态只在库内检索时参与比较。具体资格条件见上表，图中只保留各步的关键动作。

```mermaid
flowchart LR
    A["① 检查资格<br/>排除 A，B、C 入库"]
    A --> B["② 计算相似度<br/>只比较起点可见状态"]
    B --> C["③ 选择完整模板<br/>相似度影响入选概率"]
    C --> D["④ 读取所选 PIT<br/>逐步跨模板排名"]
    D --> E["⑤ 按秩分配候选<br/>连接等权十二步路径"]
```

读图：A 在第一步就被排除；B 和 C 通过资格检查后，才比较起点状态。B 更像当前，因此更容易入选；C 后来发生尖峰这一结果不能给相似度加分。第三步无放回选择完整模板，同时遵守同一研究日与事件的贡献限额。假设 B 与其他模板一起入选，第四步才读取它们的 PIT，在每个 horizon 的所选模板之间排名，第五步按 6.3 的方法重排当前候选价格。实际生成路径需要选择多条完整模板，图中没有把单条 B 当成全部场景；模板支持不足时的回退见 6.5。
...
### 6.5 从 J0 起步，再决定是否需要更复杂的路径

**先用 J0 跑通预测、优化与结算闭环，再加入 J1 对照，随后研究 J2/J3。** 这是实现与实验的顺序；最终采用哪种方法，要看验证结果。J 编号本身不表示效果越来越好。

| 实施顺序 | 使用方法 | 这一阶段要回答的问题 |
| --- | --- | --- |
| 先跑通闭环 | J0：将当前每步候选独立排列 | 给定合格的当前概率分布，能否正确生成路径、进行风险感知决策并核算结果？这一步不依赖历史 PIT 模板库。 |
| 加入简单历史对照 | J1：历史完整误差序列加到当前 P5MIN | 直接借用历史误差金额和形状，能取得怎样的预测与运营表现？ |
| 检验时间依赖的价值 | J2，再 J3 | 保持当前每步候选不变，借历史秩连接路径是否改善 J0？按相似状态选择模板是否进一步改善 J2？ |

J1 会改变每步价格候选及其分布，因此作为单独的端到端基线。J0、J2、J3 的比较则共用逐步候选、场景数和随机种子协议，用来判断连接方式的价值。

**运行 J3 时，足够相似的历史模板不够怎么办？**

J3 需要从合格历史库中选出多条完整模板。有时与当前状态相似的案例太少；有时记录虽然很多，却集中在少数几天或同一次尖峰。后者也不能提供足够多样的历史依据。例如，相隔五分钟的两个一小时窗口大部分重叠，一次尖峰可能出现在几十个窗口里，不能把这些窗口当成几十次独立事件。

因此，保存时保留所有合格的五分钟历史起点，选择时无放回抽取完整模板，并限制同一研究日、同一连续价格事件的贡献。“支持不足”指按这些规则仍无法选出所需数量和来源多样性的模板。

下面用“需要 100 条模板、最相似的案例只能提供 40 条”说明处理顺序。数字仅为教学假设，具体数量、相似度距离和放宽档位尚需在训练／验证期确定。

1. 先在与当前状态最接近的合格案例中检索。应用日／事件贡献限额后，只能选到 40 条，无法满足本轮需要。
2. 按事先规定的档位放宽相似度距离，允许稍微不那么像当前的合格案例进入候选。这里“扩大邻域”就是扩大可接受的状态差异范围。
3. 如果所有允许的放宽档位都试过，仍不够，则尝试 J2：使用一般合格 PIT 模板，不再按当前状态相似程度优先选择，仍遵守资格和贡献限额。
4. 如果 J2 也无法满足要求，再依次检查能否使用合格的 J1 残差块、J0。J1 必须有足够的合格历史残差块；J0 必须有合格的当前概率分布。每次记录实际采用的方法和回退原因。

图 6-2 把上述例子画成逐级检查流程：从左上开始，当前方法满足要求就向右生成路径，只有不足时才向下继续检查。

![图 6-2：J3 模板不足时，先放宽相似程度再逐级回退](assets/j3-template-fallback.svg)

读图：本例第一步只能提供 40 条，因此向下尝试预先配置的相似度档位；只要某档能提供满足数量与来源要求的 100 条模板，就在该档使用 J3。全部档位仍不足才检查 J2，再检查 J1 和 J0。图中每个向右的出口都是本轮选定的方法，不会同时使用全部出口；切换到 J1 时需单独标记每步价格分布可能改变。

这条回退路线用于处理已运行 J3 时的模板不足，与前面的实施顺序是两回事。放宽相似程度不能放宽结果可见时间、版本和质量要求，不能突破日／事件贡献限额，也不能把最终测试中新揭晓的结果加入历史库。不能复制模板凑数或临时减少场景数。

“预先冻结”要求在训练／验证期明确：距离怎样计算、按哪些档位放宽、什么情况下算模板不足，以及何时切换方法；进入最终测试后按这些规则执行，不能根据测试收益临时修改。上述具体参数尚未确定，正式运行前必须写入配置。每次选择还需记录不同历史起点数、涉及日数与事件数、模板距当前的时间、相似度距离，并汇总回退比例，以便看出某个方法实际上用了多少独立历史依据。

回退到 J1 后，每步候选价格及其分布可能改变，必须单独标记，不能把这部分结果仍作为“只改变时间连接”的 J3 对照。非法 CDF 或来源无法追溯属于数据／模型错误，应按第 10 章处理，不能当作普通模板不足而继续回退。

**每轮生成多少条路径，怎样确定？**

场景数 `S` 是每轮交给优化器的未来一小时价格路径数量。例如 `S=100` 表示 100 种可能走势，每种包含未来十二个五分钟价格；增加 `S` 不会改变预测步数。原始场景等权，每条路径代表 `1/S` 的概率质量。

路径太少可能漏掉重要价格范围，或让风险计算与充放电动作过度依赖少数路径；路径太多又会增加计算成本。因此，在验证期尝试不同数量，例如 100、200、400，并检查下表四项。这些数量仅为候选规模的教学例子，不是已选定配置。

| 检查项 | 具体看什么 | 为什么影响场景数 |
| --- | --- | --- |
| 每步价格分布的近似程度（边际近似） | 在每个 horizon，这些等权候选能否较好表示当前 CDF 的分位数、负价与高价概率，以及均值等摘要？ | 用少量离散价格表示一个分布可能过于粗糙，尤其容易遗漏小概率尾部。 |
| 最坏结果是否有足够路径表示（尾部样本） | 在风险损失最坏的一小部分概率质量中，有多少条路径参与计算？ | 例如等权场景下，95% CVaR 在 `S=100` 时涉及最坏 5 条路径，在 `S=400` 时涉及最坏 20 条；风险定义见 7.3。更多路径可细化计算，但不保证风险估计已经可靠。 |
| 当前动作是否稳定 | 固定当前输入和 SOC，增加场景数或更换抽样种子后，第一步充放电方向、功率与风险估计是否仍大幅变化？ | 若结果随少数路径剧烈变化，就需要检查场景数、模板支持和求解精度，不能直接采用该配置。 |
| 计算时间是否可接受 | 每轮生成路径和求解需要多久，整个历史回放需要多少计算资源？ | 场景数增加后的精度收益必须与计算成本一起评估。 |

目标是在验证期选择一个分布近似、尾部表示和动作稳定性达到预设要求，且计算成本可接受的场景数；这些要求和最终数量都要在最终测试前冻结。J0、J2、J3 的正式对照使用相同 `S`。

增加生成路径数量能细化数值近似，却不能增加历史上独立发生过的尖峰事件。历史尾部证据不足时，单纯增大 `S` 不能解决问题。人为设计的极端路径可以单列为压力测试包，但不能赋予无依据的发生概率，也不能混入期望收益或概率评分。

**什么结果才值得保留 J2/J3？**

J2 要检验历史连接关系相对 J0 的价值；J3 还必须相对 J2 提供稳定的路径预测或风险决策增量，才值得保留额外的相似度检索。更复杂的方法不会自动产生更高收益。

还需做一个“不应因重排而改善”的检查：如果所有路径共用同一套动作，风险权重为零，约束和期末价值项都与路径无关，且现金流可以按每一步的价格分别计算后相加，那么只重排各步候选不会改变期望目标。原因是每一步的价格集合与平均值都没变；同一个动作在该步的平均现金流也不变，把各步相加后仍相同。

在这些条件下，J0、J2、J3 的首动作可能因为存在多个同样好的解或求解误差而不同，但不应出现系统性的规划价值增量。要研究时间依赖是否有帮助，需要在确实会利用整条路径差异的风险目标或信息结构下比较，再用时间外预测与运营结果判断。
...
在目标函数和第一步动作约束相同、后一种信息结构允许的计划包含前一种全部计划、且求解充分时，三种方案的最优规划目标值应满足“提前预知路径 ≥ 场景树 ≥ 共用动作”，简记为 `O2 ≥ O1 ≥ O0`。这只比较规划目标，不代表真实回放收益也一定如此；若不满足，先检查信息约束、目标口径和求解是否充分。

图 7-1 对照三种信息结构允许的动作关系。为突出信息边界，只画前三步，不表示正式优化只有三步。

![图 7-1：共用动作、场景树与提前预知路径的信息边界](assets/optimization-information-structures.svg)

读图：方框表示动作，不能把它看成价格。O0 在不同价格路径下评估共用动作；O1 图中假设直到第三步才获得可区分的观测，因此第二步仍须共用动作，实际分叉时点必须由可见信息定义；O2 则从第二步起提前知道整条未来路径，所以只作乐观诊断。所有结构当前都只执行第一步，下一轮重新规划。
...
图 7-2 展示窗口内的预计现金流如何与窗口末估值共同影响当前动作，并标出它们与实际回放收益的边界。

![图 7-2：一小时窗口、窗口末状态与终端价值边界](assets/terminal-value-horizon.svg)

读图：`Π_k,s` 表示场景 `s` 中第 `k` 步扣除退化及配置成本项后的预计净现金流，`V_t` 评价窗口结束时剩余电量的延续价值；两者组成单个场景的规划价值。完整优化目标还要与安全策略比较，按场景权重计算平均增量，并扣除第 7.3 节定义的 CVaR 风险惩罚。窗口末电量保留场景下标作为通用表示；首版 O0 使用共用动作与相同电池动力学，因此各场景末电量相同。每轮估值只参与规划，回测结算只记录实际执行的现金流，整个回测段的期末状态另按共同规则处理。
...
图 8-1 把上面的功率例子画成来源与去向，虚线标出站点与电网之间的 NMI 计量边界。

![图 8-1：站点内部能量流与电表边界结算](assets/nmi-energy-settlement-boundary.svg)

读图：负荷的 4 kW 由光伏 1 kW、电池 2 kW 和电网 1 kW 共同供给，只有电网的 1 kW 穿过进口计量边界。电池输出已经用于减少购电，不能再记一笔出口收入。箭头数值是五分钟区间平均功率，结算前还需乘 `1/12 h` 转成电量；本例没有画出其他工况的充电、弃光和上网箭头。电池效率影响内部能量变化，NMI 按交流侧实际进出口计量，退化等成本按配置另计。
...
图 9-1 将时间切分、跨界样本排除和最终测试期间的更新边界放在一起。

![图 9-1：训练测试时间隔离与滚动回放的冻结边界](assets/train-test-freeze-timeline.svg)

读图：上层先确定各数据段的用途；中层以 00:00 为切分边界，说明前一段的预测目标不能跨入后一段，隔离范围还需考虑结果可见时间、模型准备耗时及更长延续窗口；下层区分测试前冻结的参数和测试中持续更新的输入与电池状态。测试新结果可在满足可见性后用于预注册的近期特征与评价，但不能用来重训、重校准或扩充模板库。图中的可选独立校准段不免除其他方案使用时间外校准预测的要求。

完整日期案例见[时间切分、滚动起点与 Test 冻结](explain/时间切分_滚动起点_Embargo与Test冻结.md)。

### 9.3 基线各自回答哪个问题

先比较预测本身，检查复杂模型是否超过简单做法。下表用 B 编号记录预测基线。第 5 章的条件经验残差模型 M0 在这里编号为 B3：M0 标记模型方案，B3 标记它在比较中的角色，二者指同一种方法。

| 预测基线（编号） | 做法与比较用途 |
| --- | --- |
| 最近价格持久性（B0） | 把最近可见的 NSW1 实际价格作为各步预测，检查是否超过“价格暂时不变”的简单判断 |
| 日内/周内季节性（B1） | 使用历史相同时段或星期的价格规律，检查是否只学到了日历效应；历史窗口和统计规则在验证期冻结 |
| 原始 P5MIN（B2） | 直接使用当时可见的官方预测，作为点预测的主比较对象 |
| 条件经验残差概率（B3） | 用 M0 给 P5MIN 补概率，作为概率预测的主比较对象 |
| 事件基础率（B4） | 用合法历史样本中负价、高价的发生比例报概率，检查事件模型是否超过简单频率统计 |

再比较完整储能策略，检查预测是否改变了动作和净价值。下表用 C 编号记录策略；同一份预测如何用于优化，也属于策略配置。

| 储能策略（编号） | 做法与比较用途 |
| --- | --- |
| 不动作/无电池（C0） | 在 S1 中保持电池不动作，在 S2 中回放无电池站点，提供基础价值参照 |
| 本地价值策略（C1） | 仅执行第 8.1 节的本地用电、光伏与备用规则，检查市场响应额外带来什么 |
| 历史阈值规则（C2） | 根据训练与验证期确定的价格阈值决定充、放或等待，检查优化是否超过简单规则；阈值及所用价格信号必须冻结 |
| 原始 P5MIN 滚动控制（C3） | 用原始 P5MIN 规划动作，作为运营价值的主比较对象 |
| 概率基线控制（C4） | 把 B3 的概率预测交给路径生成与优化环节，检查补上概率后的价值 |
| 候选概率控制（C5） | 使用候选模型的概率预测，与 C4 比较模型替换的影响 |
| 完美预知上界（C6） | 允许使用未来实际价格求解，估计资产与费用约束下的机会空间 |
...
降级是预先定义的另一条策略分支。它不能获取更新输入、沿用旧计划、填零或读取未来价格来掩盖失败。先验证配置与血缘，再验证状态与预测，最后计算并保存区间结算明细。下表区分预测方法与完整策略，回退时分别记录实际使用了什么。

| 发生什么 | 本轮如何处理 | 状态和报告如何处理 |
| --- | --- | --- |
| 候选分布非法或校准组件缺失 | 原预测标失败，以独立批次使用合格概率基线 B3，并执行对应策略 C4 | SOC 继续；回退预测不计入纯候选评分 |
...
先取约 50 个覆盖普通、负价、高价和不同初始 SOC 的验证期决策点，测建模与求解时间、可行率、最优性间隙、内存及失败类型。最优性间隙表示当前可行解与求解器给出的目标界之间还差多少，用于判断求解是否充分。再用分层验证样本选择场景数和有限主配置。求解器名称、单次时限、CPU 小时、实际经过的墙钟时长、最大并行数和内存都由实测冻结；未定义预算不启动全年回放。
...
资产敏感性覆盖效率、额定/可用能量、功率能量比、初末 SOC、备用、进出口限制；另覆盖费用、退化、概率校准误差、风险权重、尖峰捕获和通信/执行延迟。若要判断 60 分钟方案是否不显著差于更长预测方案，必须预先注册允许的性能差距，即非劣效界限，不能从 Test 反推“60 分钟足够”。
