...
| 版本 | v1.11 |
| 编制日期 | 2026-08-26 |
| 最近修订 | 2026-08-30：v1.11 为正文章节补充承上启下的章导语，新增决策时序、P5MIN vintage、边际概率、滚动 MPC、终端价值、结算回放、实验门和实施路线图；正文仅保留最近一次修订，历史版本以仓库记录为准 |
| 市场事实核验日期 | 2026-08-26 |
...
本章先概括第一阶段从 AEMO 原始数据到 NSW1 概率预测、价格路径、储能决策、NMI 结算和样本外评价的完整闭环，再列出已冻结的主决策、方案重点、成功标准、非目标与当前仓库状态。这样安排是为了让读者先区分研究边界、待验证假设和交付判据，再进入后续技术契约；本章的摘要不替代后文的定义和证据。读完本章后，应能判断各模块在闭环中承担什么任务、最终要交付什么证据；第 1 章将把这张阅读地图展开为可执行的时间、标签、资产和评价契约。
...
| 主时序 | 五分钟候选 issue time 为首区间开始；主实验通过提前冻结 `decision_cutoff = issue_time - decision_delay` 保持 h=1 可执行 |
...
| 主价格版本 | 审计后冻结的 DISPATCHPRICE 月度归档/修订最终 RRP；as-published 仅作可得性诊断或敏感性 |
...
截至编制日，仓库已经建立研究协议、文档、uv 环境和 Git 版本控制，锁定 Python 3.11、nemseer 1.0.7 与 nemosis 3.8.1；尚无 `src/`、`tests/`、数据目录、模型、优化器或回测实现。进入实现后，实验 manifest 必须记录真实 Git commit；若实验包含未提交变更，还应记录工作区状态和源码包 SHA-256，不能只用 HEAD 冒充实际运行代码版本。

---

## 1. 问题定义与固定契约

第 0 章收敛了第一阶段的研究范围和成功标准，本章把这些方向写成每一轮预测、场景生成、储能决策与回测都必须遵守的固定契约。内容依次定义预测与决策问题、时间语义和可见性、NSW1 RRP 标签与价格版本、合成储能参数、核心研究问题以及 30/60/120 分钟视野边界；这些定义是后续数据对齐、公平比较和物理/结算核算的共同基准。最终，本章产出统一的时间主键、标签单位、资产状态和评价边界，并将其交给第 2 章压缩为实现时可检索的关键设计决策。

图 1-1：第一阶段单轮决策的信息截点与十二步预测时序。

![第一阶段单轮决策的信息截点与十二步预测时序](assets/decision-information-timeline.svg)

读图：只有不晚于 `decision_cutoff_aest` 可见的数据能进入当前 snapshot；`model_issue_time_aest=t` 同时是首个执行区间的开始，当前轮只执行 `(t,t+5]`，其余 11 步用于评估当前动作的机会成本。
...
第一阶段主实验采用以下可执行时序，避免 `decision_cutoff`、`model_issue_time` 和 h=1 执行口径分叉：

```text
model_issue_time_aest = t                 # 五分钟网格点，也是首个执行区间开始
first_target_interval_end_aest = t + 5min # h=1，区间 (t, t+5]
decision_cutoff_aest = t - decision_delay_seconds
```

`decision_delay_seconds` 是数据到达、解析、预测、优化和执行准备的保守总延迟，由 E0 数据审计后冻结。主实验不把动作发布时间推迟到 `t` 之后，而是把信息截点提前到 `t - delay`，从而在主样本中保持 h=1 对应 `(t,t+5]` 可执行。所有 P5MIN 锚点、特征、站点预测和历史模板均必须满足 `available_at <= decision_cutoff_aest`。

若审计或压力实验需要显式模拟运行完成时间，则另设 `action_ready_at_aest` 轨道：当 `action_ready_at_aest` 晚于首区间执行截止时，按预注册跳步或安全动作规则处理，并单独报告。该延迟压力轨道不得与主 h=1 可执行样本混作同一结论。
...
price_version       = frozen_monthly_archive_or_revision_final（主实验，审计后冻结）
```

`RRP` 是模型和基础市场结算映射使用的价格字段；`ROP`、`APCFLAG`、市场暂停和修订字段作为诊断及制度状态保留，不能与 `RRP` 混用。

第一阶段主实验默认使用数据审计后冻结的 AEMO DISPATCHPRICE 月度归档或修订最终 RRP 版本，预测评价和结算现金流使用同一 `price_version`。若可重建首次发布或 as-published 价格，则作为诊断/敏感性单独运行；若只能取得一种可靠版本，也必须在数据快照中显式声明。价格版本改变时，新建数据快照和实验，不覆盖旧结果。
...
本章只汇总容易反复讨论的决策，不重述后续章节的技术契约。若局部规则未另行收紧，以下结论默认适用；实现与验收以表中指向的详细章节为准。

这些结论把第 1 章的固定契约压缩为实现时可检索的上位约束；第 3 章将据此落实模块职责、版本血缘和 champion/challenger 边界，后续新增方法也必须在同一约束下说明其比较对象与证据要求。

| 关键争议 | 冻结结论 | 简要理由 | 详细契约 |
|---|---|---|---|
| 历史数据能否按名义运行时间直接回放 | 不能；任一输入必须在 `decision_cutoff_aest` 前可见，历史回放使用 as-of snapshot 和合格历史库 | nominal run time 不等于数据已可用，否则会引入未来信息 | 1.2、4.4–4.9 |
| 是否先建路径或 MPC，以后再补概率校准 | 不能；先验证边际概率和完整 CDF，再生成路径并进入优化 | P10/P50/P90 不等于完整分布，边际失准时路径重排无法修复 | 6.2–6.11、7.1、11.1 |
| 条件 PIT/Schaake 是否预设为核心方法 | 不是；J0/J1 为简单基线，J2/J3 为依赖结构候选，J3 只能凭时间外增量晋升 | J3 只改变跨 horizon 的连接方式，不改善当前边际，且受历史支持度限制 | 7.1–7.10 |
| 复杂模型、多阶段树、CVaR 或 V2 是否必然优于简单方法 | 不是；只有在共同样本和相同信息集下提供稳定样本外增量才晋升，否则简化或停止 | 复杂度本身不是研究成果 | 3.4、7.10、11.8、14.2 |
| 其他 NEM 区域能否成为标签、路径或结算价格 | 不能；只能作为决策时可见的辅助特征或条件检索状态，并必须做消融 | 第一阶段的标签、资产与结算边界均为 NSW1 | 1.3、5.2、11.2 |
| 一小时视野能否先验认为足够 | 不能；60 分钟是唯一正式任务，30/120 分钟是配对敏感性，只有通过预注册非劣效判断才能称“足够” | 10 kW / 20 kWh 资产的充满时间约两小时，一小时窗可能遗漏跨小时机会 | 1.6、9.1、11.7 |
| 一小时窗末是否应强制回到固定 SOC 或将剩余电量价值设为零 | 都不作为主方案；窗口内使用终端价值，连续回测段终点再采用共同公平处理 | 零终端价值导致近视放空，每轮强制回 SOC 又会压制跨小时套利 | 1.4、8.9、9.1–9.8 |
| 离线回测能否直接支持真实项目收益结论 | 不能；合成资产、参数化费用、完美预知和离线回测都必须带明确结论边界 | 当前无真实站点、合同、执行和结算证据 | 0.5、11.8、14.3 |

---

## 3. 总体架构与模块边界

第 1 章定义了问题和边界，第 2 章把容易反复讨论的选择冻结为决策索引；本章进一步回答这些选择如何落到可实现的模块、接口和产物链中。内容涵盖端到端架构、模块职责、版本化血缘以及 champion/challenger 关系，因为只有责任边界和追溯路径清楚，后续的预测、优化和收益结论才可能回到当时可见的信息和具体配置。最终，本章形成从原始数据到联合评价的实现蓝图，并把首要的输入可信性问题交给第 4 章展开。

### 3.1 端到端架构

图 3-1：第一阶段从 AEMO 原始数据到预测—路径—决策联合评价的端到端架构。
...
读图：数据可见性、边际概率、联合路径、随机决策和结算评价依次建立在上游合格产物之上；任何下游收益都必须能够沿版本化产物链回溯到原始输入和当时的信息截点。
...
第 3 章给出了从数据到评价的模块边界，但这条链能否成立，首先取决于每个输入在决策时是否真正可见、可定位、可复算。本章从已核验的一手市场事实和权威边界出发，依次规定数据源、不可变原始层与 manifest、历史 available time 策略、Snapshot/P5MIN vintage 选择、标准化长表、缺失处理、数据审计和时间切分。这样安排是为了在模型建样本之前先排除未来信息、版本错配以及时间和单位语义错误；最终，本章形成通过 Point-in-Time 审计的共同样本和可追溯的 P5MIN vintage 体系，供第 5 章据此定义特征、基线和公平比较。
...
图 4-1：同一目标区间的 P5MIN vintages 如何进入当前 snapshot。

```mermaid
flowchart TB
    T["同一 target interval<br/>存在多个 P5MIN vintages"] --> A["A：较早 run<br/>available_at ≤ cutoff"]
    T --> B["B：中间 run<br/>available_at ≤ cutoff"]
    T --> C["C：最新可见 run<br/>available_at ≤ cutoff"]
    T --> D["D：后续 run<br/>available_at > cutoff"]
    K["decision_cutoff_aest"] --> G{"可见、有效<br/>且覆盖该 target？"}
    A -->|合格| G
    B -->|合格| G
    C -->|合格| G
    D -->|截点后才可见| X["排除出当前 snapshot<br/>不得用于当前预测"]
    G --> S["合格候选 A / B / C<br/>按冻结规则排序"]
    S --> O["选择 C：cutoff 前<br/>最新可见且有效的 vintage"]
    O --> M["写入当前 snapshot"]
    A --> R["未选 vintages 仍保留<br/>用于 revision 与血缘审计"]
    B --> R
```

读图：选择键是同一 `target_interval_end_aest`，资格判据是 `source_available_at_aest <= decision_cutoff_aest`；后续发布的 D 即使覆盖同一目标也不能进入当前信息集，未被选中的合格版本则继续保留用于修订轨迹和审计。
...
- NSW1、`INTERVENTION=0`、主价格版本可用性以及 as-published 诊断版本是否可重建；
...
数据审计（§4.8）冻结共同可用区间后，按时间顺序将样本切成四段，禁止随机切分：
...
| 数据段 | 范围 | 用来做什么 | 冻结时机 |
|---|---|---|---|
| Train 训练段 | 共同可用区间起点至 Validation 起点前（扣除相邻隔离间隔），至少覆盖一个完整年度 | 拟合基础预测模型参数、特征填补参数（§4.7）和仅允许由 Train 估计的统计量；候选时段/价格状态划分、回退阈值和收缩规则交由 Validation 选择 | 选型期间仅在 Train 内按候选配置重拟合；Validation 选定结构后，仍仅用 Train 生成最终基础模型，并在 Calibration 开始前冻结 |
| Validation 验证段 | Train 之后、Calibration 之前 | 只做选择和晋升判定，不向最终基础模型或校准器直接拟合参数：模型族、特征集、窗口长度、超参数、状态划分、回退/收缩规则、校准窗口与更新频率；B3 challenger 和 J3 晋升也在此判定 | 全部选择与晋升规则在 Calibration 开始前冻结 |
| Calibration 校准段 | Validation 之后、Test 之前 | 对已选定且冻结的基础结构产生的时间外预测拟合最终校准参数：边际分位数与事件概率校准（§6.8）、V1 允许的终端价值统一折扣或层级收缩（§9.4.3–§9.4.4）；V2 折扣仍按 §9.5 由 Validation 冻结 | 进入 Test 前冻结，Test 期不再拟合 |
| Test 测试段 | 数据审计确认的最新完整 12 个月 | 只评价、不学习：冻结模型、校准器、特征、超参数和选择规则，不吸收 Test 标签重新拟合 | 全程冻结 |

切分主键为五分钟滚动起点 `model_issue_time_aest`；每个起点的输入资格仍由 `decision_cutoff_aest` 与 available time 判定。“最新完整 12 个月”以 `model_issue_time_aest` 所属月份界定；只有当每个纳入起点的 `h=1...H` 标签、冻结价格版本和结果资格状态均已完整可见时，该月才算完整；月末起点的最后一个 horizon 可以跨入下月或下一年。

主实验采用上表的严格 Train-only 基础模型拟合口径。如研究 Train+Validation 重拟合，必须作为预注册的独立实验版本，显式定义时间外 Calibration 预测的生成规则，不得覆盖主实验结论。

各段具体边界在数据审计冻结共同可用区间后确定并写入实验配置；在此之前属于待定义参数，不得开始选型。

横切规则：

- 相邻集合之间设置不短于最大评估 horizon 的隔离间隔（embargo）：至少排除前一段中任一 `target_interval_end_aest` 不早于后一段首个 `model_issue_time_aest` 的起点，并继续按 `outcome_available_at_aest` 检查结果资格；数据发布延迟要求更长间隔时不得只使用名义 horizon。运行 120 分钟敏感性时隔离间隔至少 120 分钟；
- 滚动窗口、重训和校准更新频率必须写入实验配置；
- 主 Test 同时冻结历史依赖库：PIT 模板库（§7.5–7.8）的成员、PIT 内容、历史上下文和治理元数据，以及 J1 历史残差块（§7.4）；只纳入首个 Test cutoff 前已合格且不含 Test 目标标签的条目，仍遵守隔离间隔；
- 最终测试期若被反复查看，应记录为探索性使用，须另设未触碰留出集（untouched holdout）才能恢复强结论。

为避免“冻结模型”和“历史月度快照”混淆，三类流程分开定义：

| 流程 | 作用 | Test 期规则 |
|---|---|---|
| 主预测流水线（§6） | 对 Test 每个 cutoff 生成当前 NSW1 概率分布 | Test 前已选定并冻结最终 `forecast_snapshot_id`（版本命名见 [`schema/README.md`](schema/README.md)）、模型、校准器、特征 schema、超参数和选择规则；Test 期间只更新当时可见输入，不重训、不重校准。 |
| 历史 PIT/残差库构建（§7.4–7.8） | 为 J1/J2/J3 提供 Test 前已合格的历史依赖模板 | 可在 Train/Validation/Calibration 历史上按预注册的滚动起点（rolling-origin）月度快照重建 as-of CDF；进入 Test 前冻结库成员、PIT、历史上下文和治理元数据，Test 标签不得入库。 |
| 在线更新研究 | 模拟未来生产式滚动重训、重校准或增库 | 另立实验和独立留出集，不覆盖第一阶段冻结主 Test 结论。 |

因此，Test 期间仍每五分钟读取新的 P5MIN、市场特征和 SOC 并重新预测/优化；冻结的是模型参数、校准器、选择规则、库内容和评价边界。

完整时间线、embargo 边界手算、Test 前两轮五分钟回放以及三类流程的直观区别，见 [`explain/时间切分_滚动起点_Embargo与Test冻结.md`](explain/时间切分_滚动起点_Embargo与Test冻结.md)。

---

## 5. 特征、基线与实验公平性

第 4 章形成了共同、可追溯且遵守可见性规则的样本，但同一份样本仍可能因特征和比较口径不同而产生不公平结论。本章先固定必须实现的预测基线，再按 P5MIN、近期 NSW1、供需网络、其他区域辅助、日历制度和数据质量组织特征，最后规定变换、防泄漏和共同样本比较规则。这样可以把“模型看到了什么”和“模型相对什么变好”同时说清楚；本章产出的是供后续残差概率模型直接消费的特征契约、基线集合和公平评价门，下一章将在此基础上构建一小时边际概率分布。
...
B3_v0 是第一阶段首个必须落地的概率基线，目标是先形成可校准、可复算的 P5MIN 残差分布，而不是追求最细分组。

| 项 | B3_v0 默认规则 |
|---|---|
| 残差定义 | `actual_rrp_aud_per_mwh - anchor_rrp_aud_per_mwh`，锚点为 cutoff 前最新可见 P5MIN。 |
| 主分组 | `horizon_step + target_slot_bucket + anchor_price_regime`。slot 和价格状态边界只用 Train/Validation 冻结。 |
| 回退层级 | 主分组 → `horizon_step + target_slot_bucket` → `horizon_step` → 全局 Train 残差池。 |
| 分位数 | 使用 `forecast.internal_quantiles` 网格；分位数估计、最小样本数、收缩权重和回退阈值在 Validation 冻结。 |
| 校准 | 同时报告未校准 B3 与校准后 B3；候选 M1 的主概率比较对象是校准后 B3。 |
| 事件概率 | 负价/高价概率优先由 B3 CDF 推导；B4 事件基础率作为独立对照。 |
| 失败处理 | 关键锚点或标签缺失时不填补；分组样本不足按回退层级处理并记录实际层级。 |

B3_v0 的 challenger 可以增加季节、revision regime 或更细价格状态，但只能在 Validation 证明校准和稳定性改善后进入 Test 主配置。
...
第 5 章已经冻结了可见特征、P5MIN 基线和公平比较规则，但这些规则仍只规定输入与比较方式，尚未给出可供场景生成的完整不确定性。本章在同一 Point-in-Time 信息集上把 P5MIN 作为已知锚点，估计每个 horizon 的残差及负价/高价事件风险，再通过分位数单调化、事件概率与 CDF 融合、尾部处理、滚动校准和 OOD 降级，形成统一的概率输出接口。可验证产出是版本、覆盖率、宽度、事件校准和质量标志均可追溯的 h=1...12 边际分布，而不是单纯更窄的区间；第 7 章将据此恢复十二步价格路径的跨时间依赖。

本章正文只保留残差概率预测的技术契约。教学性解释、类比和手算示例已拆分到 [`explain/`](explain/README.md)，仅用于帮助理解，不替代市场事实来源或实验依据。

图 6-1：从 P5MIN 锚点到可供场景生成消费的 NSW1 边际概率分布。

```mermaid
flowchart LR
    A["Point-in-Time 信息集<br/>P5MIN NSW1 锚点 A(t,h)<br/>可见特征 X(t,h)"] --> B["残差定义<br/>E(t,h) = Y(t,h) - A(t,h)"]
    B --> C["残差分位数模型<br/>M0 / M1<br/>加回锚点得到价格分位数"]
    B --> D["事件概率模型 M2<br/>负价概率 / 高价超越概率"]
    C --> E["分位数单调化<br/>记录 crossing 与修正量"]
    E --> F["CDF_fusion_v0<br/>分位数为主骨架<br/>事件概率作一致性诊断"]
    D --> F
    F --> G["滚动校准与尾部处理<br/>同时检查覆盖率、宽度与 Brier"]
    G --> H{"分布合法且<br/>校准产物合格？"}
    H -->|是| I["h=1...12 校准边际 CDF<br/>P10 / P50 / P90 + 内部网格<br/>事件概率与质量标志"]
    H -->|否| J{"P5MIN 锚点与<br/>合格 B3 CDF 可用？"}
    J -->|是| K["显式降级到 B3<br/>记录 OOD / fallback"]
    K --> I
    J -->|否| L["该 cutoff 不进入主样本<br/>记录原因且不填补锚点"]
```

读图：先估计 P5MIN 的条件残差和事件概率，再把二者整理为单调、一致、经过校准且带降级标志的 CDF；第 7 章只消费通过该边际门的输出。
...
本节 M0 对应 5.1 节的概率基线 B3；本节 M2 指事件概率模块，11.2 节实验矩阵中的 M2 则指“M1 + 事件概率模块”的组合配置。第 13 章使用 P0–P6 表示实施阶段，不与模型 M 编号复用。
...
上述流程定义最终候选的选型与主 Test 评估，不授权用最终模型、校准器回填历史 PIT。历史模板另按 4.9 和 7.5 的 rolling-origin 月度快照规则构建：每个快照只能使用其生效前可见的拟合、内部选择和校准数据；若某个 family 是后期 Validation 才确定的，早期回算只能标记为探索性或从方法选择信息已可用后的起点开始积累合格库。支持不足按 7.8 节降级。
...
#### 6.6.1 CDF_fusion_v0 最小规则

CDF_fusion_v0 的目标是给优化器一个稳定、可消费的分布，同时避免事件模型冲突成为首版工程阻塞点。

| 项 | CDF_fusion_v0 默认规则 |
|---|---|
| 主骨架 | 单调化并校准后的 M1 分位数构成基础 CDF；B3 流水线同理使用其经验分布/CDF。 |
| 插值与尾部 | P01–P99 内按 6.7 冻结插值；尾部按训练期 NSW1 残差尾部和当时有效 floor/cap 处理。 |
| M2 用途 | M2 事件概率先作为校准诊断和软约束，不默认强行扭曲 M1 CDF。 |
| 冲突标记 | 若 M2 与 CDF 推导的事件概率差异超过 Validation 冻结阈值，输出 `cdf_event_inconsistency_flag`，主优化仍使用基础 CDF。 |
| 硬约束 | 分位数非交叉、概率范围、floor/cap 支持域、多个高价阈值概率单调必须满足；不满足则失败或降级到合格 B3 CDF。 |
| 报告 | 同时报告 CDF 推导事件概率、M2 事件概率、差值、冲突比例和分层 Brier/reliability。 |

将事件节点纳入带权 isotonic projection 或其他联合投影的方案编号为 CDF_fusion_v1，只能作为 Validation challenger；节点权重、容忍阈值和投影损失函数必须在 Test 前冻结。
...
分布外（Out-of-Distribution, OOD）在第一阶段表示“当前信息集相对模型拟合时的参考分布较为陌生”。OOD 是质量诊断，不等同于 CDF 数学非法，也不自动意味着扩宽区间、切换到 B3 或排除样本。诊断所依赖的原始信息必须在 `decision_cutoff_aest` 前可见，模型分歧也只能比较基于同一合格 snapshot 生成的预测；参考统计只用 Train 拟合，候选指标、阈值和聚合规则由 Validation 选择，并在 Calibration/Test 前冻结为 `ood_diagnostic_policy_id`。Test 期不得重估参考分布或调整阈值。

每轮保存分 horizon 诊断与整个 $H$ 步预测组（主任务 $H=12$）的聚合诊断，不得只保留一个最终布尔值：

| 字段 | 语义与粒度 | 最小可复算契约 |
|---|---|---|
| `feature_range_flags` | 单个当前特征是否超出 Train 参考范围；按 `(issue, horizon, feature)` 记录 | 保存特征名、当前值、变换后尺度、参考上/下界、越界方向和参考版本；边界估计方法在 Validation 冻结。缺失值由 `missingness_shift` 处理，不当作数值越界。 |
| `missingness_shift` | 当前普通特征的缺失向量或特征组缺失率相对 Train 是否异常 | 保存缺失特征列表、当前组内缺失率、Train 参考率/距离、阈值和 flag。RRP、P5MIN 锚点等关键缺失仍按 4.7 节快速失败，不得被该分数吸收成普通 OOD。 |
| `anchor_revision_anomaly` | 当前选中的 P5MIN 锚点相对同一 target 的上一个合格可见 vintage 修订是否异常；按 horizon 记录 | 保存前后 run 键、修订量 A$/MWh、对应 Train 参考组、异常分数/阈值和 flag。没有上一个合格 vintage 时记为 `not_applicable` 及原因，不得记为零修订。 |
| `price_regime_rarity` | 当前锚点价格状态或 $H$ 步锚点曲线形态在 Train 中的支持程度 | 保存 regime/bucket ID、参考样本数与频率/稀有度、最低支持阈值和 flag；分组边界只用 Train/Validation 确定并冻结。它表示联合状态稀有，与单特征越界分开报告。 |
| `model_disagreement` | 候选 CDF、B3 CDF 及事件概率模块之间的预测冲突；它是预测争议诊断，不单独证明输入分布外 | 在冻结的分位数网格上保存按 horizon 的 CDF/分位数距离、负价/高价概率差、聚合方法、阈值和 flag。某模块不可用时显式记录 `not_applicable`/失败原因。 |
| `ood_score` | 对上述可用组件的整组综合陌生程度，分数方向必须固定为“越大越陌生”或在政策中明示反向 | 保存各组件原始值与标准化值、聚合公式/权重、取值范围、组件缺失处理、阈值、最终 flag 和政策版本。`ood_score` 不得覆盖或替代组件明细。 |

输出动作按下列边界处理：

1. 仅 OOD flag/score 超阈值、但候选 CDF 仍满足硬合法性与冻结校准契约时，原预测带诊断标志输出，不自动切换 B3。
2. 只有在 Validation 证明某种单调扩宽能改善覆盖且不过度扩大区间后，才允许形成 `ood_widening_policy_id`；启用时同时保存扩宽前/后分位数与触发原因。
3. 候选 CDF 违反 6.6.1 节硬约束或冻结校准产物不合格时，才按图 6-1 显式降级到合格 B3 CDF，并分别记录 `fallback_reason` 和 OOD 诊断；不得把高 `ood_score` 本身伪装成数学失败。
4. 主评价中 P5MIN 锚点缺失的 cutoff 不用降级模型填入；运营安全动作与降级轨道在单独样本中报告。

诊断记录的主键、组件明细、参考版本和降级字段见 [`schema/README.md#411-ood-diagnostic-record`](schema/README.md#411-ood-diagnostic-record)。
...
| 质量与生成 | OOD 组件诊断及政策版本、fallback 状态/原因、其他 quality flags、生成时间 |
...
第 6 章已为每个未来步提供边际概率，但逐步看待这些边际仍无法说明尖峰会持续多久、负价是否连片或价格如何恢复。本章在不改变当前边际候选集合的前提下构造并比较联合路径：保留 J0/J1 等简单基线，同时检验历史 PIT/Schaake 依赖、条件检索、模板资格、去集中化和冷启动回退。可验证产出是带版本、模板来源、权重、边际不变量和质量标志的一小时 NSW1 场景批次；它将作为第 8 章随机 MPC 的不确定性输入，并由后续运营回放检验依赖结构是否真正带来价值。

### 7.1 目的与术语边界

对每个 issue time，路径工厂把 12 个已校准边际连接成 $S$ 条 NSW1 一小时场景：
...
第 7 章产出的是对未来价格不确定性的路径表达，还没有说明在当前电池和站点状态下应该执行什么动作。本章把场景与 SOC 连续性、功率/能量、效率、S1/S2 站点与 NMI、费用、退化、风险偏好和信息结构结合，规定随机 MPC 的求解与安全边界；核心循环是每五分钟重算一次，但只执行当前根节点的第一步。可验证产出是可复算的首动作、全窗口计划、风险/约束解释、求解状态和 fallback 记录，而不是把后续 11 步当成已批准计划；第 9 章将进一步确定窗口末库存的终端价值，第 10 章再用实际五分钟结果结算和回放。

随机 MPC、O0/O1/O2 与 CVaR 的教学解释见 [`explain/终端价值与随机MPC.md`](explain/终端价值与随机MPC.md)。

图 8-1：五分钟滚动随机 MPC 闭环。

```mermaid
flowchart LR
    A["决策 cutoff t<br/>实际 SOC / NMI 状态<br/>资产、费用与站点配置"] --> B["构建 Point-in-Time snapshot<br/>选择 cutoff 前可见的<br/>P5MIN vintage 与输入"]
    B --> C["生成当前 12 步联合场景<br/>按 O0 / O1 信息结构组织"]
    C --> D["求解 12 步随机 MPC<br/>S1 / S2 物理约束<br/>终端价值与 CVaR"]
    D --> Q{"求解有效？"}
    Q -->|是| E["只执行根节点第 1 步<br/>充电 / 放电 / 待机<br/>executed=true 仅此步"]
    Q -->|否或超时| F["采用冻结安全动作<br/>记录失败、fallback<br/>与质量标志"]
    F --> E
    E --> G["区间结束后观测实际<br/>RRP、负荷 / PV 与执行结果"]
    G --> H["更新 SOC 与 NMI<br/>结算当前五分钟并保存状态"]
    H --> I["下一个 cutoff t+5<br/>丢弃旧的后续计划"]
    I --> A
```

读图：每轮用 cutoff 时可见的信息规划 12 步，却只执行当前首个五分钟动作；随后用实际观测更新状态并重新规划，求解失败也通过显式安全动作进入同一条连续状态轨迹。
...
在明确的进口/出口功率限制下，充电计为进口、放电计为出口。S1 的回放输入、输出和报告措辞按 10.4 节执行。

### 8.5 S2 优化侧站点与 NMI 模型
...
优化器不另定义价格和费用口径，而是复用 10.1 节的统一结算映射：将场景 RRP 代入同一 `fee_config_version`，得到场景进口价 $p^{imp}_{k,s}$ 与出口价 $p^{exp}_{k,s}$。五分钟场景现金流为：
...
RRP 为负时不自动截断；进口或出口是否完整传导负价由 10.1–10.2 节的费用契约决定。
...
第 8 章已经规定 terminal value 在场景目标中如何被消费，但还没有规定它怎样构造、冻结、验证和在数据不足时回退。本章把窗口外机会成本收敛为统一、可替换的终端价值接口，依次比较零终端价值 V0、训练期历史延续表 V1 与具备 Point-in-Time 数据门控的 Predispatch 延续价值 V2，并处理连续回测段的期末公平。可验证产出是带资产、费用、版本和血缘信息的 terminal value artifact，以及对 SOC、期末调整和实际现金流的诊断与回退规则；这些产物将在第 10 章的真实五分钟回放中检验，而不应被当作已实现收入。

终端价值的直观解释见 [`explain/终端价值与随机MPC.md`](explain/终端价值与随机MPC.md)。

图 9-1：一小时窗口、窗口末状态与终端价值边界。

![一小时窗口、窗口末状态与终端价值边界](assets/terminal-value-horizon.svg)

读图：窗口内现金流和窗口末终端价值共同影响当前首动作，但 terminal value 只是对视野外增量机会成本的近似；连续回测只结算实际执行区间，并在整段终点按共同规则进行一次期末调整。
...
#### 9.4.4 V1_v0 定位与防偏乐观规则

V1_v0 定位为 **terminal salvage approximation**：用 Train 历史实际未来价格构造的延续价值近似，经 Validation/Calibration 折扣和收缩后进入 MPC。它不是可执行未来策略，也不是额外已实现收入。

| 项 | V1_v0 默认规则 |
|---|---|
| 数据 | 原始价值表只用 Train；Validation 选择 context、延续窗口、SOC 网格、平滑和候选折扣；Calibration 可校准统一折扣或层级收缩；Test 完全冻结。 |
| 折扣 | 折扣满足 `0 <= gamma_ctx <= 1`；若折扣/收缩未完成校准，V1 只能作为 challenger，主终端价值回退到 V0 或预注册固定 salvage。 |
| 口径 | 与主 MPC 使用相同资产、效率、费用、退化、NMI 和最终 SOC/salvage 边界。 |
| 诊断 | 必须报告终端价值占目标函数比例、SOC 贴边率、期末现金调整占已实现 PnL 比例、相对 V0 的现金流改善和去极端日稳定性。 |
| 回退 | 若出现长期囤电、SOC 长时间贴上界、收益主要来自期末调整、或实际五分钟现金流相对 V0/Vfixed 恶化，则 V1_v0 不得作为主终端配置。 |

V1_v0 的作用是缓解一小时窗口近视；若其增量主要来自完美预知延续优化而非可复现的首动作改善，应降级为诊断或改用可实施 rollout 价值表。
...
第 8 章和第 9 章定义了在预测场景和终端价值下如何形成首动作，但运营价值只能在实际历史结果上逐个五分钟结算验证。本章将预测侧场景价格与回放侧冻结 `price_version` 下的实际 NSW1 RRP 分开，通过统一进出口映射及显式费用、退化口径，分别回放 S1 纯价格信号、S2 合成站点和 C0–C6 反事实策略；同时保存 SOC、负荷/PV、NMI、求解失败、干预和价格修订等状态，保证每笔现金流可追溯。可验证产出是连续事件级回测明细、策略与反事实的净价值/风险/约束结果，以及仅用于机会空间诊断的完美预知对照；第 11 章据此进行配对统计、PnL 归因和 Go/No-Go 验收。

S1/S2、RRP 到进出口价格和费用口径的教学解释见 [`explain/表后结算与S1S2.md`](explain/表后结算与S1S2.md)。

图 10-1：实际五分钟回放中的物理能量流与现金流汇合。

```mermaid
flowchart TB
    A["决策 cutoff t<br/>冻结策略与首个五分钟动作"] --> B["执行当前五分钟区间<br/>S1 / S2"]
    B --> P["区间结束观测<br/>实际 NSW1 RRP<br/>冻结 price_version"]
    B --> E["区间结束观测与执行结果<br/>电池动作、负荷、PV<br/>S1 代理 / S2 outcome"]
    E --> F["能量守恒与状态更新<br/>SOC、充放电、NMI 进 / 出口"]
    F --> R["五分钟物理能量记录<br/>进口 / 出口 kWh<br/>吞吐与退化基础"]
    P --> Q["统一价格与费用映射<br/>RRP / 1000<br/>fee_config_version"]
    R --> S["五分钟结算与记账<br/>进口成本、出口收入<br/>费用、退化、辅助功耗"]
    Q --> S
    S --> T["事件级回测明细<br/>strategy × decision<br/>现金流、物理状态与质量标志"]
    T --> U["连续段累计结果<br/>净价值、风险、回撤与 regret<br/>期末调整按共同规则只记一次"]
```

读图：实际物理能量与冻结价格/费用分别形成结算输入，再在同一五分钟事件中按 NMI 进出口能量计算现金流并更新状态；因此任何累计收益都能回溯到具体动作、能量和价格版本。
...
该映射是优化与实际结算的唯一价格契约：第 8.8 节对每条场景使用场景 RRP，滚动回放则使用冻结 `price_version` 下的实际 NSW1 RRP；两者必须绑定同一费用配置版本，不能分别维护两套公式或符号约定。
...
### 10.4 S1 与 S2 的回放和报告边界

S1/S2 的优化侧物理定义见 8.4–8.5 节；本节只规定实际回放与结论措辞。

| 场景 | 回放与结算输入 | 必须保存和报告的边界 |
|---|---|---|
| S1 纯价格信号 | 实际 NSW1 RRP、已执行电池动作和参数化 NMI 边界；无负荷/PV | 保存资产、SOC、进出口、吞吐和现金流；只称“价格信号下的运营代理价值” |
| S2 合成表后站点 | 决策时使用站点 forecast，执行后使用独立保存的负荷/PV outcome 结算 | 分别保存负荷、PV、电池、NMI 和现金流；不能称“典型 NSW 用户”或真实户用/工商业收益 |

S2 只有在 S1 数据、模型和优化链稳定后才进入主实验。除明确的确定性合成假设外，forecast 与 outcome 不得共用未来实现轨迹。
...
名义 P5MIN run 不代表数据已经到达。主实验按 1.2.1 使用冻结的 `decision_delay_seconds` 把信息截点提前，保持 h=1 为 `(t,t+5]` 的可执行首区间。E0 至少审计并冻结：

- 可见时间代理；
- 数据下载/解析、预测、优化和执行准备的保守总延迟；
- h=1 可执行主轨道与显式 `action_ready_at_aest` 延迟压力轨道的切换规则。

延迟压力轨道若导致 h=1 已不可执行，必须按预注册跳步或安全动作规则单独报告；不得与主 h=1 可执行结果混作同一结论。
...
第 10 章已经把预测、场景、滚动动作、实际状态和五分钟结算连接成可回放的运营闭环；本章进一步回答“什么证据才足以支持一个候选方法晋升”。内容从 E0–E8 的分阶段实验出发，再用预测、路径、决策与终端矩阵、配对统计、非劣效判断、联合成功判定、PnL 归因和分层报告区分预测改善、路径改善与实际结算价值。这样安排是为了避免仅凭单一预测指标或少数尖峰收益提前选型，并确保所有比较使用相同的信息集、资产状态、费用情景和 Test 冻结规则；本章最终产出各阶段的 Go/No-Go 结论、主比较结果和不确定性报告，供第 12 章固化为接口、配置和可追溯产物。

图 11-1：E0–E8 从数据可信性到费用后运营证据的分阶段验收门。

```mermaid
flowchart TB
    E0["E0 数据审计<br/>共同样本、可见性与价格版本"] --> G0{"数据门通过？"}
    G0 -->|否| R0["停止强运营结论<br/>修复数据或只交付审计结果"]
    G0 -->|是| E13["E1–E3 边际概率<br/>B3、残差模型、校准与完整 CDF"]
    E13 --> G1{"边际概率可信？"}
    G1 -->|否| R1["修复或保留简单 B3<br/>不进入联合路径收益主张"]
    G1 -->|是| E4["E4 联合路径<br/>J0 / J1 / J2 / J3"]
    E4 --> G2{"路径不变量、支持度<br/>与时间外证据通过？"}
    G2 -->|否| R2["冻结 J0 / J1 等简单路径<br/>记录回退后继续验证"]
    G2 -->|是| E57["E5–E7 决策与视野<br/>O0 / O1 / O2、CVaR、V0–V2、30/60/120"]
    R2 --> E57
    E57 --> G3{"物理、信息结构、终端价值<br/>与视野证据通过？"}
    G3 -->|否| R3["降级到安全信息结构或简单终端处理<br/>收紧结论边界"]
    G3 -->|是| E8["E8 S2 与费用<br/>合成站点、Low / Base / High、退化"]
    R3 --> E8
    E8 --> G4{"概率、运营和风险证据<br/>能够联合支持晋升？"}
    G4 -->|是| O1["报告完整候选改进<br/>并保留离线研究边界"]
    G4 -->|否| O2["报告部分改善或负结果<br/>选择更简单基线"]
```

读图：No-Go 不都代表项目整体失败；它可能要求停止强结论、修复上游，也可能意味着保留已验证的简单基线继续完成后续对照，具体规则以本章实验矩阵和第 14 章停止条件为准。
...
| E4 路径方法 | J0/J1/J2/J3 谁能形成更真实且有用的一小时路径 | E3 | 按第 2 章决策索引与 7.10 节晋升或简化 |
...
编号说明：本表 B3 对应 6.3 节的模型 M0；本表 M2 是“M1 + 事件概率模块”的组合实验名，不是说事件模型替代了 M1。第 13 章的 P0–P6 是实施阶段编号。
...
第 11 章定义了用哪些实验和统计证据判断方法是否晋升，本章把这些证据落实为可实现、可复算、可审计的工程契约。内容从建议模块划分和通用 horizon 接口开始，进一步规定场景长表、Terminal value artifact、实验 run manifest、配置校验与快速失败、错误边界、依赖与命令以及最小交付物；每类产物都应能沿 `decision_id` 回溯到可见 snapshot、模型与场景版本、资产/费用/终端配置以及源码和环境哈希。这样做是因为离线研究的可信度不仅取决于指标，还取决于同一输入能否重跑、错误能否被发现、结果能否解释；本章最终形成供第 13 章分阶段实现和测试的接口与产物基础，而不是已经完成的生产系统。
...
这是建议逻辑结构，不要求一次性创建全部目录。优先按 P0–P6 实施阶段逐层建立，每个模块依赖上游稳定数据契约，不把下载、建模、优化和结算写进一个 notebook。
...
- B3_v0 分组、回退层级、样本阈值和收缩规则已冻结；
- CDF_fusion_v0 的事件冲突阈值、硬约束和降级规则已冻结；
- OOD 候选诊断规格在 Validation 前完整，选定的 `ood_diagnostic_policy_id`、Train 参考快照、组件阈值与聚合规则在 Calibration/Test 前冻结；扩宽政策未通过 Validation 时必须显式禁用；
- 所有时区和 interval semantics 显式；
- 资产安全/运营边界顺序正确；
- 费用数值、单位、方向和来源非空；Gross-RRP 除外但须明确 `diagnostic_only=true`；
- V1_v0 折扣、诊断阈值、回退条件以及 V1/V2 的资产、费用与主 MPC 配置兼容；
...
第 12 章已经定义接口、配置和产物的形状，本章把它们排成一条可执行的实施路线。P0–P6 从工程与版本前置、数据审计与共同样本、预测与运营基线、残差模型与概率校准、联合路径、随机 MPC 与终端价值，逐层推进到 S2、费用和最终样本外评价；每个阶段同时规定交付物、退出门以及相应的时间、概率、场景、优化、结算和重放测试。这样安排是为了让下游复杂度建立在上游证据之上，并在失败时回退到已验证的简单基线，而不是让一次端到端收益结果掩盖数据、概率或物理错误；本章产出的是可排期、可复现、可回退的第一阶段实施与测试计划，完成后由第 14 章审查剩余风险、停止条件和结论边界。

图 13-1：P0–P6 按上游证据逐层推进的实施路线。

```mermaid
flowchart TB
    P0["P0 工程与版本前置<br/>环境、配置、哈希与 run 目录"] -->|复现与环境门| P1["P1 数据审计与共同样本<br/>vintage、可见性、NSW1 标签"]
    P1 -->|可追溯与无未来信息门| P2["P2 预测与运营基线<br/>B0–B4、C0 / C2 / C3 / C6"]
    P2 -->|公平比较、守恒与首步回放门| P3["P3 残差模型、校准与完整分布<br/>M1 / M2、CDF、事件概率"]
    P3 -->|Validation 选型与 Test 冻结门| P4["P4 联合路径<br/>J0 / J1 / J2 / J3、PIT 治理"]
    P4 -->|可见性、不变量、支持度与联合评分门| P5["P5 随机 MPC 与终端价值<br/>O0 / O1 / O2、CVaR、V0 / V1 / V2"]
    P5 -->|信息结构、SOC/NMI、求解与终端价值门| P6["P6 S2、费用与最终样本外评价<br/>C0–C6、费用、退化与配对统计"]
    P6 -->|manifest 可复算且结论边界完整| O["第一阶段交付<br/>结果、测试证据、局限与建议"]
    P0 -. 退出门未通过 .-> F["修复当前阶段、回退上游<br/>或冻结已验证的简单基线<br/>通过相应门后再继续"]
    P1 -. 退出门未通过 .-> F
    P2 -. 退出门未通过 .-> F
    P3 -. 退出门未通过 .-> F
    P4 -. 退出门未通过 .-> F
...
读图：P0–P6 是带退出门的证据链，不是无条件向前的开发清单；第 13.8–13.14 节的测试分别约束相应阶段，并在关键错误出现时阻止下游复杂度继续累积。
...
退出门：每种方法通过不变量、可见性、复现和联合评分；路径选择按第 2 章决策索引与 7.10 节晋升或简化。

### 13.6 P5：随机 MPC 与终端价值
...
- B3_v0 分组、样本不足回退、收缩和校准前/后输出符合冻结配置；
- 所有分位数有限且单位为 A$/MWh；
- 单调修复后无 crossing，原始 crossing 仍可审计；
- 事件概率在 $[0,1]$；
- 高价阈值嵌套和分位数/CDF 一致；
- CDF 插值单调、逆 CDF 有界；
- CDF_fusion_v0 在事件冲突超阈值时输出 flag 且主优化分布按冻结规则降级/保留；
- 市场 floor/cap 按目标时点有效版本使用；
- 校准器没有读取 Test；
- OOD 参考统计只用 Train，候选阈值和聚合规则只用 Validation 选择，Test 期参数与政策版本不变；
- 合成的特征越界、缺失模式偏移、P5MIN 修订异常、稀有价格状态和模型分歧样本只触发各自诊断，组件不适用不被写成零分；
- 仅 OOD 超阈值不会自动扩宽或降级 B3；启用冻结扩宽政策时扩宽前/后输出、触发原因和 policy ID 均可追溯；
...
- V1_v0 折扣满足范围约束，诊断阈值和回退条件按冻结配置触发；
- $V(E_{ref})=0$ 的归一化正确；
- 深夜值不被午夜置零支配；
- SOC 网格插值相对更细网格误差可控；
...
第 13 章给出了逐层实施和测试门，本章从“如何实现”转向“何时不应继续，以及结论能说到哪里”。内容先登记数据、可见性、尾部样本、模板、场景树、求解、终端价值、费用、合成站点和统计选择等风险，再把失败情形对应到停止、简化、回退或降级报告，最后列出最终报告必须披露的区域、价格版本、信息集、资产、费用和不确定性边界。这样做是为了让负结果和无法验证的假设成为正式交付物，而不是在获得不稳定收益后再临时补充免责声明；本章产出风险登记、停止/简化决策和结论边界，只有通过这些边界的方向才进入第 15 章讨论未来扩展。
...
第 14 章已经限定了第一阶段的风险、停止条件和可外推范围，本章只讨论在这些证据成立后可以开启的演进方向。内容依次覆盖更长预测 horizon、五区域联合场景、复杂模型以及从离线研究到生产控制的前置条件；每一项都要求新的数据可见性、共同样本、概率与运营证据以及必要的安全和合规审查，不能因为接口可扩展就默认纳入当前项目。这样安排是为了把“未来可能做什么”与“第一阶段已经证明什么”分开，避免范围扩张或把离线合成结果包装成生产能力；本章产出一组带准入条件的演进门槛，而不是承诺的路线图，正文结束后由附录提供配置、流程伪代码、签核清单和参考资料。
...
time:
  ...
  decision_delay_seconds: REQUIRED_AFTER_AUDIT  # 用于 decision_cutoff = issue_time - delay
...
target:
  ...
  price_field: RRP
  intervention: 0
  price_version: REQUIRED_AFTER_AUDIT  # 主实验默认冻结月度归档/修订最终 RRP
  unit: AUD_PER_MWH
...
forecast:
  ...
  probability_baseline_version: B3_v0
  b3_v0:
    grouping: [horizon_step, target_slot_bucket, anchor_price_regime]
    fallback_order:
      - [horizon_step, target_slot_bucket, anchor_price_regime]
      - [horizon_step, target_slot_bucket]
      - [horizon_step]
      - global_train_residual_pool
    min_samples_and_shrinkage: REQUIRED_BEFORE_VALIDATION
  ...
  cdf_fusion_version: CDF_fusion_v0
  event_inconsistency_thresholds: REQUIRED_BEFORE_VALIDATION
  ood_diagnostic:
    candidate_policy_specs: REQUIRED_BEFORE_VALIDATION
    policy_id: REQUIRED_BEFORE_CALIBRATION  # 选定后映射为 ood_diagnostic_policy_id
    reference_period: train_only
    selected_component_thresholds_and_aggregation: REQUIRED_BEFORE_CALIBRATION
    widening_policy_id: DISABLED_UNLESS_VALIDATED
...
terminal_value:
  ...
  v1_diagnostics_and_fallback_thresholds: REQUIRED_BEFORE_TEST
...
| 结论边界 | 60 vs 120、完美预知、合成资产、参数化费用、离线回测和负结果均按第 2 章决策索引、11.8 节和 14.3 节的边界措辞报告。 |
...
本附录汇总本方案依赖的项目内契约、AEMO/AEMC 一手资料和方法资料。项目内契约用于约束范围、数据、预测、储能与实验实现；官方资料用于核验市场字段、时间和规则，方法资料用于说明概率路径与风险优化方法。各资料的适用性和核验日期仍以正文中的具体标注为准，本附录不将第三方方法资料提升为市场事实来源。
