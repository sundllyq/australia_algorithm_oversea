...
| 版本 | v1.10 |
| 编制日期 | 2026-08-26 |
| 最近修订 | 2026-08-30：v1.10 压缩正文重复内容，区分优化与结算章节职责，并将实施里程碑改为 P0–P6；正文仅保留最近一次修订，历史版本以仓库记录为准 |
| 市场事实核验日期 | 2026-08-26 |
...
| 流程 | 作用 | Test 期规则 |
|---|---|---|
| 主预测流水线 | 对 Test 每个 cutoff 生成当前 NSW1 概率分布 | Test 前已选定并冻结最终 `forecast_snapshot_id`、模型、校准器、特征 schema、超参数和选择规则；Test 期间只更新当时可见输入，不重训、不重校准。 |
| 历史 PIT/残差库构建 | 为 J1/J2/J3 提供 Test 前已合格的历史依赖模板 | 可在 Train/Validation/Calibration 历史上按预注册 rolling-origin 月度快照重建 as-of CDF；进入 Test 前冻结库成员、PIT、context 和治理元数据，Test 标签不得入库。 |
| 在线更新研究 | 模拟未来生产式滚动重训、重校准或增库 | 另立实验和 holdout，不覆盖第一阶段冻结主 Test 结论。 |
