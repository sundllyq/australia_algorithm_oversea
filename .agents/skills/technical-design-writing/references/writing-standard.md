...
Markdown 中的流程、时序和状态关系优先用 Mermaid；需要精确呈现数值的研究图使用绘图库。交付格式不支持 Mermaid 时，改用可渲染的 SVG、PNG 或简洁文本图，并保留可编辑源文件。无需为配图引入网页应用或图片生成工具。
...
图前说明它回答什么问题，图后指出应读出的关键关系。交付前尽可能检查实际渲染，确认没有截断、重叠、不可读标签，且箭头、时间、单位与正文一致；未实际渲染时，不宣称完成视觉验证。配图不能替代对设计原因和适用条件的文字说明。
